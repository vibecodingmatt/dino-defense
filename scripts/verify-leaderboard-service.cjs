...
(async()=>{
  ...
  await p.evaluate(()=>{save.settings.mute=true;startLevel(0,'fresh',1);G.wave=100;victory();});
  ...
  fs.writeFileSync(path.join(out,'verification.json'),JSON.stringify({checkedAt:new Date().toISOString(),frontend:process.argv.includes('--live')?'published':'local at Pages origin',api,maps:7,touchSubmission:true,publicReadback:true,errors},null,2));
  ...
  if(player){
    ...
    execFileSync(process.execPath,[cli,'d1','execute','dino-defense-leaderboard','--remote','--command',`DELETE FROM scores WHERE player = '${player}'`],{cwd:path.join(root,'leaderboard'),windowsHide:true,stdio:'pipe'});
