...
(async()=>{
 ...
 await phone.locator('#btnArsenal').tap();await phone.locator('.ag-weapons button[data-key="missile"]').tap();await phone.locator('.ag-tiers button').last().tap();await phone.screenshot({path:path.join(out,'arsenal-guide-phone.png')});
