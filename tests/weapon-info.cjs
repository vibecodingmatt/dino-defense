...
(async()=>{
 ...
 for(const [name,width,height]of [['phone',390,844],['small-phone',320,568],['phone-landscape',844,390],['tablet',768,1024],['tablet-landscape',1024,768],['large-tablet',1366,1024]]){
  ...
  const map=await page.evaluate(()=>{const r=cv.getBoundingClientRect(),s=FieldCommand.suggested();return{x:r.left+(s.x-G.cam.x)*G.cam.zoom*r.width/W,y:r.top+(s.y-G.cam.y)*G.cam.zoom*r.height/H+PLACE_LIFT_PX};});await page.touchscreen.tap(map.x,map.y);assert.equal(await page.evaluate(()=>G.towers.length===1&&G.cash===0),true,'Tap to place after reading');
 ...
 await extra.page.setViewportSize({width:320,height:568});await extra.reset();await extra.page.locator('#btnArmory').tap();const swipe=await extra.point('extinction'),scrollBefore=await extra.page.locator('#shop').evaluate(e=>e.scrollTop);await extra.touchEvent('touchStart',[swipe]);for(let i=1;i<=5;i++){await extra.touchEvent('touchMove',[{...swipe,y:swipe.y-i*15}]);await extra.page.waitForTimeout(30);}await extra.touchEvent('touchEnd');await extra.page.waitForTimeout(550);assert.equal(await extra.page.evaluate(()=>WeaponInfo.isOpen||G.placing!==null),false);assert.ok(await extra.page.locator('#shop').evaluate(e=>e.scrollTop)>scrollBefore,'Real vertical swipe should scroll the expanded bay');await extra.context.close();
