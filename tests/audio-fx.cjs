...
(async()=>{
 ...
 const mobile=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,serviceWorkers:'block'});await mobile.route('https://**/*',r=>r.abort());const mp=await mobile.newPage();mp.on('pageerror',e=>errors.push(e.message));await mp.goto(base);await mp.evaluate(()=>{save.settings.mute=false;save.settings.music=false;});await mp.locator('#btnQuickPlay').tap();await mp.waitForFunction(()=>AC?.state==='running');assert.equal(await mp.evaluate(()=>effectAudio().stats().maxVoices),18);console.log('PASS: Play touch gesture unlock and smaller phone voice budget');
