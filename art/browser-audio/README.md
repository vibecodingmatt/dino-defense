...
`js/audio-fx.js` authors 45 original effects, each with three deterministic
...
The game routes its ten launches to `shot`, `flame`, `gas`, `snipe`, `cryo`,
`zap`, `pulse`, `missile`, `thoomp` and `novaLaunch`. Cryo impact is `frost`, artillery impact
is `shellImpact`, and Tesla chain hops use `arc`. Extinction also uses `novaCharge` for ignition and `novaImpact` for its plasma bloom. Finishers dispatch through
...
refer to the PCM payload. It contains 135 performances: 8,814,720 PCM bytes,
6,539,912 bytes compressed. The authored source is also the fallback if fetch or
...
headless desktop Chrome. The encoded bank also occupies about 8.4 MiB in memory.
...
actual firing and impact routes for all ten weapons, deaths of all 33 species
